...
- Use ~window-cursor-type~ where available during cursor hiding, to
  target hiding only to the window being scrolled (e.g. if the same
  buffer is visible in multiple windows).
